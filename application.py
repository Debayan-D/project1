...
def register():
    ...
    if request.method == "POST":
        ...
        if db.execute("SELECT * FROM users WHERE username = :username", {"username": username}).rowcount != 0:
           ...
           db.execute("INSERT INTO users (firstname, lastname, email, username, password) VALUES (:firstname, :lastname, :email, :username, :password)", {"firstname": firstname, "lastname": lastname, "email": email, "username": username, "password": password })   
           db.commit()
...
def login():
    ...
    if request.method == "POST":
        ...
        if query_user[4] == username and query_user[5] == password:
...
def result(isbn):
    ...
    if request.method == "POST":
        ...
        if db.execute("SELECT * FROM review WHERE username_id = :username_id AND isbn_id = :isbn_id", {"username_id": username_id, "isbn_id": isbn_id}).rowcount == 0:
           ...
           return render_template("result.html", search=True, reviews=reviews, book_result=book_result, good_reads=good_reads)
        else:
